...
export function RecentFinancialActivities() {
  ...
  // Filter activities to only show finance-related ones for financial users
  const financialActivities = allActivities.filter(activity => 
    activity.type === 'finance' || 
    activity.entity_type === 'inventory' ||
    activity.entity_type === 'order' ||
    activity.action.toLowerCase().includes('budget') ||
    activity.action.toLowerCase().includes('finance') ||
    activity.action.toLowerCase().includes('inventory') ||
    activity.action.toLowerCase().includes('order')
  );

  // Only show first 5 activities initially unless showAll is true
  const displayedActivities = showAll ? financialActivities : financialActivities.slice(0, 5);
  const hasMoreToShow = financialActivities.length > 5;

  return (
    <Card className="border-border/50 bg-card/80">
      <CardHeader>
        <CardTitle className="text-md font-semibold">Financial Activities</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          {isLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              No financial activities to display
            </div>
          ) : (
            <>
              <Timeline>
